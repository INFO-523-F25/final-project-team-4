...
This project was designed to analyze daily Air Quality Index (AQI ) data from the United States Environmental Protection Agency. This analysis explored both geographical, seasonal, and weather-related features to understand their impact on the AQI value for the area. With the findings of the analysis, an interactive dashboard was created to explore the trends and visualize more complex relationships. 
...
Since the temperature and wind seemed to have the greatest impact on the linear regression model, these features need to be visualized in the dashboard. The visualizations chosen for the dashboard were designed to answer the guiding questions mentioned above. The heatmap of the United States provides the user with an overall perspective of the AQI values during a given month across the United States. By seeing the distribution of the AQI values, the user can identify groups of state that has similar AQI values due to similar geological features and weather patterns. The next visual provides trend lines of the average temperature, wind, and AQI over time for a selected state.  This visual allows the user to identify any seasonality patterns between these variables and the AQI values. The final visual is a heatmap that visualizes the average AQI values across the selected state’s counties and across each month. This visual helps highlight any complex geological factors across counties that may be influencing the AQI values. 

Based on these visualizations, there were a couple of observations that were worth noting. When looking at the heatmap of the United States (Figure 1), the southwest has higher AQI values than the rest of the United States. This aligns with the linear regression model since the southwest consists of mostly warmer states.  However, it is interesting to see that Arkansas had a significantly higher AQI in January than its surrounding states. Another interesting observation was found when looking at the distribution of the AQI values across counties in Arizona (Figure 2),. There is a significant difference between Maricopa County and the other counties. This difference could be explained by the geographical differences between the counties. 

::: {#fig-heathamp_usa}
![United State Heatmap](images/AQI%20USA%20Heatmap.png){width="60%"}
:::
::: {#fig-heathamp_county}
![Arizona County Heatmap](images/AQI%20County%20Heatamp.png){width="60%"}
:::


## Discussion/Conclusion

Through this analysis, the interactions between the AQI values and external factors are more complex than a linear relationship. As a result, in order to better understand the features that better predict the AQI trends, the incorporation of other geographical features, such as elevation, could enrich our dataset more. Furthermore, additional historical AQI values for each state would help with the accuracy of the model and classification of the influential predictors. 
